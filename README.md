...
- [ ] **Diagnosticar la descarga por lotes** — `update_prices_daily.py` falla ("possibly delisted; no timezone found") al usar `yf.download` con varios tickers por llamada. Se actualizó `yfinance` a 1.7.0 y se reescribió el script para descarga por lotes (`BATCH_SIZE`, split nuevo/existente, merge incremental) — probado en local con datos sintéticos y con pruebas manuales de `yfinance` en el servidor, todo correcto. **Pero en la ejecución real en el servidor sigue fallando igual que antes** — pendiente conseguir el traceback exacto del fallo real para seguir diagnosticando (la sospecha es que el caso sintético no reproduce algo específico del universo de tickers real).
- [x] ~~Detección de novedades + resumen diario con IA local (email inteligente)~~ — hecho: nuevo paso `daily_digest.py` en el pipeline, entre el screener y el envío. Compara el CSV de hoy contra el snapshot del día anterior (`data/snapshots/`) sin usar IA para detectar los hechos (entradas/salidas de la lista de calidad, cambios de P/E o D/E ≥15%, cruces de precio objetivo), y solo si hay algo genuinamente nuevo llama a un LLM local (Ollama, `llama3.1` 8B, corriendo en el Thinkcentre) para redactar esos hechos en lenguaje natural — el LLM nunca inventa datos ni da recomendaciones, solo redacta lo ya calculado, con fallback a lista de hechos en crudo si Ollama no responde. Todo este paso corre en la fase de cálculo de las 22:00, así que el texto ya está listo antes del envío de las 7:00, que se limita a leer `data/daily_digest.html` y `data/email_decision.txt` y enviar (o no).
  - **Top movers, comentados uno por uno con su valor/calidad**: cada mover del día (top 5 subidas/caídas) se comenta SIEMPRE (no solo si es nuevo) con su P/E, P/B, ROE, D/E, FCF Yield y precio objetivo si están disponibles (`_mover_quality_commentary` en `novelty_detector.py`, datos añadidos a `top_movers_report.csv` en `value_quality_screener.py`) — para poder valorar si una caída pinta a oportunidad o una subida ya se ha comido el recorrido, no solo el % de movimiento. Como consecuencia práctica, el top movers del día casi siempre genera al menos un "hecho", así que el email dejó de depender solo de eventos raros (entradas/salidas de calidad) para dispararse — llegará la inmensa mayoría de los días laborables, siempre con este análisis por empresa.
  - **Anomalías fuera de la IA**: `AnomalyDetector` se sigue ejecutando y `data/anomalies_report.csv` se sigue generando y adjuntando al email, pero deliberadamente ya no se convierte en "hecho" para el LLM — no aparece en el texto redactado, solo en el CSV para quien quiera mirarlo a mano.
- [ ] **(Opcional) Puntuación combinada Valor+Calidad por acción** — idea propuesta por otro modelo de IA: combinar las métricas de Valor y Calidad en una única puntuación por empresa (percentiles de P/E, P/B, ROE, D/E, FCF Yield), y usar el cambio día a día de esa puntuación como señal adicional de novedad (vía `novelty_detector.py`). Sentido: sería aditivo, nunca sustituiría las alertas por métrica individual, y siempre iría acompañada de su desglose (nunca una puntuación desnuda). Pendiente de decidir: peso igual entre las 5 métricas o dar más peso a alguna de las dos mitades (Valor vs Calidad) antes de construirlo.
...
4. **Detecta novedades y redacta el resumen** (`daily_digest.py`) — guarda un snapshot de los 3 CSV del día y los compara con el snapshot del día anterior (`novelty_detector.py`, pura aritmética: altas/bajas en la lista de calidad, cambios de P/E o D/E ≥15%, cruces de precio objetivo, movers que coinciden con la lista de calidad, anomalías repetidas). Si no hay ninguna novedad real, se marca el email de hoy como "SKIP" y el proceso termina aquí — no llega nada a las 7:00. Si hay novedades, se le pasan como hechos ya calculados a un LLM local (Ollama, `llama3.1` 8B en el Thinkcentre) que solo las redacta en lenguaje natural — nunca inventa datos ni da recomendaciones — y el resultado queda guardado en disco listo para enviar.

**Fase de envío (7:00):**
5. **Envía el informe por email** (`send_email_report.py`) — si el paso anterior decidió "SEND", construye el HTML (tabla estilizada, nombre de empresa junto al ticker, movers, el resumen del día redactado por el LLM, y el CSV completo adjunto) y lo envía; si decidió "SKIP", no se conecta siquiera al SMTP. Soporta varios destinatarios (`EMAIL_TO` separado por comas). Este paso es casi instantáneo porque todo el trabajo pesado (incluida la llamada al LLM) ya se hizo en la fase de las 22:00.
